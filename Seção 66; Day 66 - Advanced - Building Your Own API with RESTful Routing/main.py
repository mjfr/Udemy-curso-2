...
def search():
    ...
    cafes_dict = [cafe.to_dict() for cafe in cafes]
    if len(cafes_dict) == 0:
        return jsonify(response={"Not Found": "Sorry, we don't have a cafe at that location."})
...
def add():
    ...
    return jsonify(response={"Already Exists": "The cafe you're trying to add already exists in the database."})
