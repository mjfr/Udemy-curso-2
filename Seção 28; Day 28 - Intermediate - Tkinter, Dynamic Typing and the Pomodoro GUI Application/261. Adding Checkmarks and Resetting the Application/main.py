...
def count_down(count):
    ...
    if count > 0:
        global timer
        timer = window.after(10, count_down, count - 1)  # Pega uma quantidade de tempo que o método
    # deve esperar e depois executa uma função
    else:
        start_timer()
        check_label.config(text=f"{'✔' * math.floor(reps/2)}")
...
reset_button.config(text="Reset", bg=RED, highlightthickness=0, command=reset_timer)
...
check_label.config(bg=YELLOW, fg=GREEN, font=(FONT_NAME, 20, "bold"))
